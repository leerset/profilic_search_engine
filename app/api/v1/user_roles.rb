...
#### global
      ...
      put :set_global_roles do
        ...
        return data_not_found(MISSING_ROL) if Role.where(role_type: 'global', id: role_ids).count < role_ids.count
...
##### one organization join
      ...
      put :change_organization_roles do
        ...
        return permission_denied(NOT_GOD_OA_DENIED) unless current_user.god? || current_user.oa?(organization)
        role_ids = params[:role_ids].uniq
        return data_not_found(MISSING_ROL) if Role.where(role_type: 'organization', id: role_ids).count < role_ids.count
...
##### set / get organizations roles
      ...
      get :get_organizations_roles do
        authenticate!
        user = User.find_by(id: params[:user_id])
        return data_not_found(MISSING_ORG_USR) if user.nil?
        resp_ok('organization_roles' => user.organization_roles_array)
      end
      ...
      put :set_organizations_roles do
        ...
        return data_not_found(MISSING_ORG) if Organization.find_by(id: organization_id).count < organization_ids.count
        ...
        return permission_denied(NOT_GOD_OA_DENIED) unless current_user.god? || current_user.all_oa?(organizations)
