class UserEncryptionSerializer < ActiveModel::Serializer
  ...
  def home_address
    ...
    AddressSerializer.new(h_address, show_phone_number: myself? || god? || manager?) if h_address
  end

  def work_address
    w_address = object.work_address
    AddressSerializer.new(w_address, show_phone_number: myself? || god? || manager?) if w_address
  end
